...
const ChunkComponent: React.FC<ChunkComponentProps> = ({ chunk, index, bannerData }) => {
  ...
  return (
    <div className="grid grid-cols-3 w-full gap-4 mb-4 sm:mb-2 md:grid-cols-2 sm:gap-2">
      {isEvenIndex ? (
        <>
          {chunk.slice(0, 3).map((product, i) => (
            <div key={i} className="col-span-1 md:hidden">
              <ProductProfile {...product} isStatic={true} key={'group2652' + i} className={`w-full`} />
            </div>
          ))}
          {chunk.slice(0, 2).map((product, i) => (
            <div key={i} className="col-span-1 hidden md:block">
              <ProductProfile {...product} isStatic={true} key={'group2652' + i} className={`w-full`} />
            </div>
          ))}
          {chunk.slice(2, 4).map((product, i) => (
            <div key={i} className="col-span-1 hidden md:block ">
              <ProductProfile {...product} isStatic={true} key={'group2652' + i} className={`w-full`} />
            </div>
          ))}
          {chunkLength >= 5 && banner && banner != undefined && (
            <div
          ...
          {chunk.slice(3, 5).map((product, i) => (
            <div key={i} className="col-span-1 md:hidden">
              <ProductProfile {...product} isStatic={true} key={'group2652' + i} className={`w-full`} />
            </div>
          ))}

          {chunk.slice(4, 5).map((product, i) => (
            <div key={i} className="col-span-1 hidden md:block">
              <ProductProfile {...product} isStatic={true} key={'group2652' + i} className={`w-full`} />
            </div>
          ))}
